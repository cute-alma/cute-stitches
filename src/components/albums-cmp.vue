<template>
  <div class="flex flex-col justify-center md:flex-row gap-[10px]">
    <div
      @click="navToDemand"
      class="flex flex-col gap-[10px] p-[10px] bg-album-bg rounded-md h-fit w-full max-w-[333px] hover:cursor-pointer"
    >
      <p>Tus favoritos a &nbsp; <span class="font-rouge-script text-3xl">Pedido</span></p>
      <div class="grid grid-cols-3 gap-[10px] items-stretch">
        <div
          v-for="(p, index) in ondemand.slice(0, 6)"
          :key="p.name"
          class="relative cute-popup transform transition-transform hover:-translate-y-2"
          :style="{ 'animation-delay': `${0.5 + 1 / index}s` }"
        >
    ...
    >
      ...
      <div class="grid grid-cols-3 gap-[10px] items-stretch">
        <div
          v-for="(p, index) in instock.slice(0, 6)"
          class="relative cute-popup transform transition-transform hover:-translate-y-2"
          :key="p.name"
        >
          <img class="rounded-md w-full h-full" :src="loadImg(p.img)" alt="" />
